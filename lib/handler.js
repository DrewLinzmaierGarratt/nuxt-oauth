...
Handler.prototype.createAuth = function createAuth () {
  const { oauthHost, oauthClientID, oauthClientSecret, oauthScopes, authorizationUri, accessTokenUri } = Object.assign(this.opts, { authorizationUri: '/oauth/authorize', accessTokenUri: '/oauth/token' })
  const protocol = this.req.headers['x-forwarded-proto'] || this.req.headers['X-Forwarded-Proto'] || 'http'

  return new OAuth2({
    authorizationUri: `${oauthHost}${authorizationUri}`,
    accessTokenUri: `${oauthHost}${accessTokenUri}`,
    clientId: oauthClientID,
    clientSecret: oauthClientSecret,
    scopes: (oauthScopes || '').split(','),
